...
import { IFindByDocumentClientCommand } from '../../domain/commands';
import { IResponse, IClientsService } from '../../domain/interfaces';
import { ClientDomainEntityBase } from '../../domain/entities';

export class FindByDocumentClientUseCase<
  Command extends IFindByDocumentClientCommand = IFindByDocumentClientCommand,
...
> {
  ...
  private validateCommand(
    ...
    if (!command.document || command.document.trim() === '') {
  ...
  private executeInvoiceAggregateRoot(
    ...
    return this.clientsService.findByDocument(command.document);
  }
}
